...
export default function ResetPasswordPage() {
  ...
  const showSnackbarMessage = (message, type = "success", duration = 3000) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showSnackbarMessage("Password Reset Successfully!", "success");
        setTimeout(() => {
          navigate("/login");
        }, 1000);
      } else {
        showSnackbarMessage("Failed to reset password. Please try again.", "error");
      }
    } catch (error) {
      console.log(error);
